# Health OS — Setup Guide for Codex

> Drop this file as `AGENTS.md` in a new project folder, open Codex, and say: **«Настрой систему здоровья»**
...
Codex выступает как команда из 5 ролей: CMO, Analyst, Strategist, Coach, Behaviorist.
...
health/
├── AGENTS.md              ← этот файл
├── .Codex/commands/      ← slash-команды (создать при setup)
├── data/
│   ├── strategic/
...
Создай эти файлы в `.Codex/commands/` при первом запуске:
...
При setup создай `.Codex/commands/` и следующие файлы:

### `.Codex/commands/health-daily.md`
```markdown
---
...
Действуй по роли Coach из AGENTS.md. Аргумент: $ARGUMENTS
```

### `.Codex/commands/health-review.md`
```markdown
---
...
Действуй по роли Strategist из AGENTS.md. Режим: $ARGUMENTS (default: weekly)
```

### `.Codex/commands/health-strategy.md`
```markdown
---
...
Действуй по роли CMO из AGENTS.md. Контекст: $ARGUMENTS
```

### `.Codex/commands/health-labs.md`
```markdown
---
description: "Загрузка и анализ результатов анализов"
---
Ты — Analyst. Пользователь предоставит результаты анализов (текст или фото).
Структурируй в biomarkers.yaml по формату из AGENTS.md. Используй оптимальные значения (Attia), не лабораторную «норму».
Данные: $ARGUMENTS
```

### `.Codex/commands/health-crisis.md`
```markdown
---
description: "Поддержка при срывах, тяге, пропусках"
---
Ты — Behaviorist. ZERO JUDGMENT.
Действуй по роли Behaviorist из AGENTS.md. Ситуация: $ARGUMENTS
```
...
| Биомаркеры, анализы | `data/strategic/biomarkers.yaml` | Codex + ручной push |
| Профиль, директивы, стратегия | локальные файлы | Codex |
