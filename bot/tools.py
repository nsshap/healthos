...
TOOLS = [
    _fn(
        ...
        "Записать приём пищи в сегодняшний лог. Вызывай когда пользователь описывает что съел/выпил.",
        {
            ...
            "time": {"type": "string", "description": "Время приёма пищи HH:MM. ВАЖНО: передавай этот параметр ТОЛЬКО если пользователь ЯВНО назвал время еды в ТЕКУЩЕМ сообщении (например «в 14:30 я ела яйца», «вчера в 7 утра», «час назад»). Если время в текущем сообщении не указано — НЕ передавай этот параметр совсем, бот сам подставит текущее время. НИКОГДА не используй время из предыдущих сообщений диалога или из ранее залогированных meals — каждое сообщение про еду по умолчанию считается «сейчас», если не указано иначе."},
        },
    ...
    _fn(
        "get_glucose_now",
        (
            "Получить текущий уровень глюкозы из Libre 3 (CGM-сенсор). "
            "Возвращает ммоль/л, стрелку тренда (↑↑/↑/→/↓/↓↓) и возраст показания в минутах. "
            "Сам автоматически синхронизирует свежие данные из LibreLinkUp перед ответом. "
            "Вызывай когда пользователь спрашивает 'какая глюкоза?', 'сколько сахар?' или перед "
            "интерпретацией еды."
        ),
        {},
        [],
    ...
    _fn(
        "sync_glucose",
        (
            "Принудительно подтянуть последние ~12 часов показаний глюкозы из LibreLinkUp в Supabase. "
            "Обычно не нужно вызывать вручную — get_glucose_now и glucose_chart делают это сами. "
            "Используй только если пользователь явно просит 'обнови глюкозу' или подозревает "
            "что данные устарели."
        ),
        {},
        [],
    ...
    _fn(
        "glucose_around_meal",
        (
            "Проанализировать постпрандиальный ответ глюкозы на конкретный приём пищи: "
            "базовый уровень (за 30 мин до еды), пик (в течение 2 часов после), дельту, "
            "время до пика, время возврата к базе. По умолчанию анализирует последний приём пищи "
            "за сегодня. Вызывай через ~1.5-2 часа после еды, когда пользователь спрашивает "
            "'как я отреагировала на …?' или 'покажи реакцию на завтрак'."
        ),
        {
            "meal_index": {
                "type": "integer",
                "description": "Индекс приёма пищи в сегодняшнем логе: -1 = последний (по умолчанию), 0 = первый.",
            }
        },
        [],
    ),
    _fn(
        "glucose_chart",
        (
            "Сгенерировать график глюкозы за последние N часов с маркерами приёмов пищи поверх кривой. "
            "Возвращает PNG, который бот сам отправит пользователю. "
            "Вызывай когда пользователь просит 'покажи график глюкозы', 'как сахар сегодня', "
            "'график за день' и т.п. Также автоматически возвращает статистику: среднее, "
            "% времени в целевом диапазоне (3.9-7.8 ммоль/л), мин/макс."
        ),
        {
            "hours": {
                "type": "integer",
                "description": "За сколько часов назад строить график. По умолчанию 12. Максимум 24.",
            }
        },
        [],
    ),
