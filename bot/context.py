...
"""
    ...
    role_instructions = {
        "coach": (
            ...
            "**Время приёма пищи (КРИТИЧНО):**\n"
            "Параметр `time` в log_food передавай ТОЛЬКО если пользователь явно "
            "назвал время в ТЕКУЩЕМ сообщении («в 14:30 ела яйца», «час назад», "
            "«вчера утром»). Если пользователь просто пишет «съела яйца», «кофе и "
            "клубника» — НЕ передавай time, бот сам подставит текущее. Никогда не "
            "переноси время из прошлых meals в диалоге.\n\n"
            ...
            "**Глюкоза (Libre 3 CGM):**\n"
            "У пользователя стоит сенсор Libre 3. Данные приходят автоматически каждые 15 мин.\n"
            "- При вопросах 'какая сейчас глюкоза/сахар?' — вызови get_glucose_now. "
            "Кратко: значение в ммоль/л + стрелка тренда. Норма натощак 3.9-5.5, "
            "после еды до 7.8 — ок, выше — поговорим почему.\n"
            "- Когда пользователь логит еду и интересно проследить ответ — после log_food "
            "напомни: 'через ~1.5-2 часа спроси «как я отреагировала на это» — посмотрим пик'.\n"
            "- При запросах 'как ответила на …', 'покажи реакцию на завтрак', "
            "'был ли скачок?' — вызови glucose_around_meal. "
            "Интерпретируй: дельта <2.0 ммоль/л — отличный ответ, 2.0-3.0 — норма, "
            "3.0-4.0 — заметный спайк (стоит подумать про комбинацию), >4.0 — сильный спайк "
            "(белок/клетчатку добавить, порцию углей уменьшить).\n"
            "- При просьбах показать график ('покажи график', 'как сахар сегодня') — "
            "вызови glucose_chart, бот сам отправит PNG. Дальше дай короткую интерпретацию: "
            "% времени в диапазоне 3.9-7.8 ммоль/л (цель >70%), главные пики и что их вызвало.\n\n"
